...
$tglTerakhir = date('t',strtotime($tglBulan));
 ...
 <table width="100%">
 	<tr>
	 <td>
 			<img src="../../../assets/img/logo_absensi.png" width="130">
 		</td>
 		<td> 
 			<center>
...
<table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
  ...
  <tr bgcolor="<?=$warna; ?>">
    ...
    <td align="center"><?=$ds['jk'];?></td>
		...
		<?php 
		$ket = oci_parse ($con,"SELECT * FROM _logabsensi
		INNER JOIN tb_mengajar ON _logabsensi.id_mengajar=tb_mengajar.id_mengajar
		INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
		INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
...
<?php 
$sakit = oci_fetch_array(oci_parse ($con,"SELECT COUNT(ket) AS sakit FROM _logabsensi
	INNER JOIN tb_mengajar ON _logabsensi.id_mengajar=tb_mengajar.id_mengajar
		INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
		INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
...
<td align="center" style="font-weight: bold;">
<?php 
$izin = oci_fetch_array(oci_parse ($con,"SELECT COUNT(ket) AS izin FROM _logabsensi
	INNER JOIN tb_mengajar ON _logabsensi.id_mengajar=tb_mengajar.id_mengajar
		INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
		INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
...
<td align="center" style="font-weight: bold;">
	<?php 
$alfa = oci_fetch_array(oci_parse ($con,"SELECT COUNT(ket) AS alfa FROM _logabsensi
	INNER JOIN tb_mengajar ON _logabsensi.id_mengajar=tb_mengajar.id_mengajar
		INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
		INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
