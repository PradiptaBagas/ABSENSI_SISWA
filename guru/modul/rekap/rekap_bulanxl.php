...
$kelasMengajar = oci_parse ($con,"SELECT * FROM tb_mengajar 
...
foreach ($kelasMengajar as $d) 
	...
	$qry = oci_parse ($con,"SELECT * FROM _logabsensi INNER JOIN tb_siswa ON _logabsensi.id_siswa=tb_siswa.id_siswa
		WHERE MONTH(tgl_absen)='$bulan'
	 GROUP BY _logabsensi.id_siswa  ORDER BY _logabsensi.id_siswa ASC ");
foreach ($qry as $db)

	// tampilkan data walikelas
$walikelas = oci_parse ($con,"SELECT * FROM tb_walikelas INNER JOIN tb_guru ON tb_walikelas.id_guru=tb_guru.id_guru WHERE tb_walikelas.id_mkelas='$d[id_mkelas]' ");
 	...
 	<tr>
 		<td>
 			<center>
 				...
 				<h1>
 					ABSESNSI SISWA <br>
 					<small>ABSENSI KUY</small>
 				</h1>
 				<hr>
 				<em>
				 Jl. Raya Tlogomas No.246, Babatan, Tegalgondo, Kec. Lowokwaru, <br>Kota Malang, Jawa Timur 65144<br>
 				<b>Email : Beskem@gmail.com Telp.081234567890</b> 
 				</em>
...
<table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
  ...
  <tr bgcolor="<?=$warna; ?>">
    ...
    <td align="center"><?=$d['jk'];?></td>
		...
		<?php 
					$ket = oci_parse ($con,"SELECT * FROM _logabsensi WHERE DAY(tgl_absen)='$i' AND id_siswa='$d[id_siswa]' AND id_mengajar='$_GET[pelajaran]' AND MONTH(tgl_absen)='$bulan' GROUP BY DAY(tgl_absen) ");
		foreach ($ket as $h)
...
<?php 
$sakit = oci_fetch_array(oci_parse ($con,"SELECT COUNT(ket) AS sakit FROM _logabsensi WHERE id_siswa='$d[id_siswa]' and ket='S' and MONTH(tgl_absen)='$bulan' and id_mengajar='$_GET[pelajaran]' "));
echo $sakit['sakit'];
...
<td align="center" style="font-weight: bold;">
<?php 
$izin = oci_fetch_array(oci_parse ($con,"SELECT COUNT(ket) AS izin FROM _logabsensi WHERE id_siswa='$d[id_siswa]' and ket='I' and MONTH(tgl_absen)='$bulan' and id_mengajar='$_GET[pelajaran]' "));
echo $izin['izin'];
...
<td align="center" style="font-weight: bold;">
	<?php 
$alfa = oci_fetch_array(oci_parse ($con,"SELECT COUNT(ket) AS alfa FROM _logabsensi WHERE id_siswa='$d[id_siswa]' and ket='A' and MONTH(tgl_absen)='$bulan' and id_mengajar='$_GET[pelajaran]' "));
echo $alfa['alfa'];
