...
<a href="#"><?=strtoupper($d['mapel']) ?></a>
					...
					<div class="row">
						...
						<?php 
								...
								$last_pertemuan = oci_parse ($con,"SELECT * FROM _logabsensi WHERE id_mengajar='$_GET[pelajaran]' GROUP BY pertemuan_ke ORDER BY pertemuan_ke DESC LIMIT 1  ");
								$cekPertemuan = oci_num_rows($last_pertemuan);
								$jml = oci_fetch_array($last_pertemuan);
							...
							<div class="card">
								<div class="card-body">
									...
									<input type="date" name="tgl" class="form-control" value="<?=date('Y-m-d') ?>" style="background-color: white;color: black;">
									...
									<input type="hidden" name="pertemuan" class="form-control" value="<?=$pertemuan; ?>">
										...
										$siswa = oci_parse ($con,"SELECT * FROM tb_siswa WHERE id_mkelas='$d[id_mkelas]' ORDER BY id_siswa ASC ");
										$jumlahSiswa = oci_num_rows($siswa);
...
$siswa_telah_absen_hari_ini = oci_parse ($con,"SELECT * FROM _logabsensi INNER JOIN tb_siswa ON _logabsensi.id_siswa=tb_siswa.id_siswa WHERE _logabsensi.id_siswa='$s[id_siswa]' AND _logabsensi.tgl_absen='$tgl_hari_ini' AND _logabsensi.id_mengajar='$_GET[pelajaran]' AND _logabsensi.ket='' ");
if (oci_num_rows($siswa_telah_absen_hari_ini) < 1) {
								...
								<?php 
									if (isset($_POST['absen'])) {
										...
										for ($i =0; $i <=$total ; $i++) {
											...
											$cekAbsesnHariIni = oci_num_rows(oci_parse ($con,"SELECT * FROM _logabsensi WHERE tgl_absen='$today' AND id_mengajar='$pelajaran' AND id_siswa='$id_siswa' "));
											...
											if ($cekAbsesnHariIni > 0) {
												...
												$insert = oci_parse ($con,"INSERT INTO _logabsensi VALUES (NULL,'$pelajaran','$id_siswa','$today','$ket','$pertemuan')");
