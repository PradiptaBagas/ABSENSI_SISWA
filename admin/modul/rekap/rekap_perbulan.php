...
$walikelas = oci_parse ($con,"SELECT * FROM tb_walikelas INNER JOIN tb_guru ON tb_walikelas.id_guru=tb_guru.id_guru WHERE tb_walikelas.id_mkelas='$_GET[kelas]' ");
...
<i class="fas fa-print"></i>
 ...
 REKAP (<?=strtoupper($d['semester']);?> - <?=strtoupper($d['tahun_ajaran']);?>)
			...
			$qry = oci_parse ($con,"SELECT * FROM _logabsensi
...
CETAK BULAN (<?php echo strtoupper(namaBulan($bulan));?>)
	...
	$qryAbsen = oci_parse ($con,"SELECT * FROM _logabsensi INNER JOIN tb_siswa ON _logabsensi.id_siswa=tb_siswa.id_siswa
